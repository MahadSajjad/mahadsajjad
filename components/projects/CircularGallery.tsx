...
class Media {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  bend: number; textColor: string; borderRadius: number; font: string; program: any; plane: any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  title: any; scale: number; width: number; widthTotal: number; x: number; padding: number;
  speed: number; isBefore: boolean; isAfter: boolean; imageScale: number;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font, imageScale = 1 }: any) {
    ...
    this.scale = 0; this.width = 0; this.widthTotal = 0; this.x = 0; this.padding = 0;
  ...
  onResize({ screen, viewport }: any = {}) {
    ...
    this.plane.scale.y = (this.viewport.height * (750 * this.scale * this.imageScale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (900 * this.scale * this.imageScale)) / this.screen.width;
...
class App {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  planeGeometry: any; medias: Media[]; mediasImages: any[]; raf: number;
  ...
  isDown: boolean; scroll_position: number; start: number;
  ...
  constructor(
    ...
    this.medias = []; this.mediasImages = []; this.raf = 0;
    this.isDown = false; this.scroll_position = 0; this.start = 0;
    ...
    this.createGeometry(); this.createMedias(items, bend, textColor, borderRadius, font, imageScale);
  ...
  createMedias(items?: { image: string; text: string }[], bend = 1, textColor?: string, borderRadius?: number, font?: string, imageScale = 1) {
    const defaultItems = [
    ...
    this.medias = this.mediasImages.map((data, index) => new Media({
      ...
      bend, textColor, borderRadius, font, imageScale
    }));
  }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      app = new App(containerRef.current, { items, bend, textColor, borderRadius, font: resolvedFont, scrollSpeed, scrollEase, imageScale });
    });
    return () => {
      isMounted = false;
      app?.destroy();
    };
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, imageScale]);
